//! Integrator — generates Jarvis-standard SKILL.toml + SKILL.md from scout results.
...
fn sanitize_path_component(name: &str) -> Result<String> {
    ...
    if sanitized == ".." || sanitized.contains('/') || sanitized.contains('\\') {
        bail!("Skill name '{name}' is unsafe as a path component");
    }
    Ok(sanitized)
}
...
mod tests {
    ...
    fn integrate_creates_files() {
        let tmp = std::env::temp_dir().join("jarvis-test-integrate");
