...
pub fn build_system_prompt(
    ...
    if !tools.is_empty() {
        ...
        prompt.push_str("\nWhen you need to use a tool, call it using the function calling interface. Do not describe tool calls in text — invoke them directly.\n\n");
    }
    ...
    if prompt.is_empty() {
        "You are Jarvis, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
mod tests {
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(prompt.contains("Name: Jarvis"), "missing IDENTITY content");
