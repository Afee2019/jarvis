...
/// and forwards PRIVMSG messages to the `Jarvis` message bus.
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(&mut writer, &format!("USER {} 0 * :Jarvis", self.username)).await?;
...
mod tests {
    ...
    fn irc_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.username.as_deref(), Some("jarvis"));
