...
const OPEN_SKILLS_SYNC_MARKER: &str = ".jarvis-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.jarvis/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled() -> bool {
    if let Ok(raw) = std::env::var("JARVIS_OPEN_SKILLS_ENABLED") {
...
fn resolve_open_skills_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("JARVIS_OPEN_SKILLS_DIR") {
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# Jarvis Skills\n\n\
             ...
             ```bash\n\
             jarvis skills install <github-url>\n\
             jarvis skills list\n\
             ```\n",
        )?;
    }
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.jarvis");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.jarvis/skills"));
    }
