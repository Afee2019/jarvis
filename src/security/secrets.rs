...
// in `~/.jarvis/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.jarvis/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(jarvis_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: jarvis_dir.join(".secret_key"),
            enabled,
        }
    }
