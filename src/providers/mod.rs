...
pub use traits::{
    tool_spec_to_definition, ChatMessage, ChatResponse, FunctionCall, FunctionDef, ToolCall,
    ToolDefinition,
};
...
/// 3. Generic fallback variables (`JARVIS_API_KEY`, `API_KEY`)
///
...
fn resolve_api_key(name: &str, api_key: Option<&str>) -> Option<String> {
    ...
    for env_var in ["JARVIS_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
