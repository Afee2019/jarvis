...
## Benchmark Snapshot (Jarvis vs OpenClaw)
...
> Notes: Jarvis results measured with `/usr/bin/time -l` on release builds. OpenClaw requires Node.js runtime (~390MB overhead). PicoClaw and Jarvis are static binaries.

<p align="center">
  <img src="jarvis.jpeg" alt="Jarvis vs OpenClaw Comparison" width="800" />
</p>

Reproduce Jarvis numbers locally:
...
# Chat
jarvis agent -m "Hello, Jarvis!"

# Interactive mode
jarvis agent

# Start the gateway (webhook server)
jarvis gateway                # default: 127.0.0.1:8080
jarvis gateway --port 0       # random port (security hardened)
...
When an unsupported `runtime.kind` is configured, Jarvis now exits with a clear error instead of silently falling back to native.
...
Jarvis enforces security at **every layer** — not just the sandbox. It passes all items from the community security checklist.
...
> **Run your own nmap:** `nmap -p 1-65535 <your-host>` — Jarvis binds to localhost only, so nothing is exposed unless you explicitly configure a tunnel.
...
6. **Test:** Send a message to your WhatsApp Business number — Jarvis will respond via the LLM.

## Configuration

Config: `~/.jarvis/config.toml` (created by `onboard`)
...
Jarvis supports **identity-agnostic** AI personas through two formats:
...
[AIEOS](https://aieos.org) is a standardization framework for portable AI identity. Jarvis supports AIEOS v1.1 JSON payloads, allowing you to:
...
- New `Skill` → `~/.jarvis/workspace/skills/<name>/`
